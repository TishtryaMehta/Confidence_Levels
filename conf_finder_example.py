...
#ARTIFICIAL (WHITE AND RED COLORED NOISE)
noise_color=1.2
np.random.seed(0)
period=20.
start=0
...
step=3
tt = np.linspace(start, end, num=int(end/step))
t=np.linspace(0, end, num=int(end/step))
white_noise = np.random.normal( 0,1,size=tt.shape) 			
sig_raw = np.sin((2*math.pi)*t/period) 
#sig_raw_2 = np.sin(2.*np.pi*tt*(1./500.) )
col_noise= powerlaw_psd_gaussian(noise_color, len(tt))
sig_input=.5*sig_raw + .5*col_noise + 0.5*white_noise 

"""
#IF YOU WANT TO SAVE YOUR TEST SIGNAL TO TEST WITH OTHER ROUTINES
...
# PUGH PAPER: "Significance testing for quasi-periodic pulsations in solar and stellar flares" (2017), 10.1051/0004-6361/201730595
# VAUGHAN PAPER: "A simple test for periodic signals in red noise" (2005), 10.1051/0004-6361:20041453 


broken_conf(sig_input,step,P=0.95, show=True )
